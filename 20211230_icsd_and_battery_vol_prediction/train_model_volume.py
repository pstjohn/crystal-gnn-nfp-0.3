...
data = pd.read_pickle(Path(inputs_dir, "20211230_all_data_with_vol.p"))
data['volumeperatom'] = data['true_volume'] / data['num_sites']
...
def calculate_output_bias(train):
    ...
    total_energies = train["true_volume"]
...
def build_dataset(split, batch_size):
    return (
        tf.data.Dataset.from_generator(
            lambda: ((row.inputs, row.volumeperatom) for _, row in split.iterrows()),
            output_signature=(
...
model_name = "20211230_icsd_and_battery_vol_prediction"
